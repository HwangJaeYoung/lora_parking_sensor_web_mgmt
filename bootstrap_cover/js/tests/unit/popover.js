$(function () {
  ...
  QUnit.test('should throw an error when show is called on hidden elements', function (assert) {
    ...
    try {
      $('<div data-toggle="popover" data-title="some title" data-content="@Johann-S" styles="display: none"/>').bootstrapPopover('show')
    } catch (err) {
  ...
  QUnit.test('popover should call content function only once', function (assert) {
    ...
    var done = assert.async()
    var nbCall = 0
    $('<div id="popover" styles="display:none">content</div>').appendTo('#qunit-fixture')
