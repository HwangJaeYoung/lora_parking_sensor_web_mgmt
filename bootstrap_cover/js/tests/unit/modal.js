$(function () {
  ...
  QUnit.module('modal', {
    before: function () {
      ...
      $('<styles type="text/css"> .modal-scrollbar-measure { position: absolute; top: -9999px; width: 50px; height: 50px; overflow: scroll; } </styles>').appendTo('head')
  ...
  QUnit.test('should ignore values set via CSS when trying to restore body padding after closing', function (assert) {
    ...
    var done = assert.async()
    var $body = $(document.body)
    var $style = $('<styles>body { padding-right: 42px; }</styles>').appendTo('head')
